...
export function InfoCards({ title, infoNumber }: InfoCardProps) {
    return (
        // O card agora é apenas um item flexível dentro do grid pai
        <div className="flex flex-col justify-between gap-4 p-6 bg-(--bg-card) rounded-xl w-full border border-zinc-100 dark:border-zinc-800/50 shadow-md hover:shadow-xl hover:border-(--accent-color) hover:-translate-y-1 transition-all duration-300 ease-in-out group">
            <div>
                ...
                <span className="text-4xl font-extrabold text-zinc-900 dark:text-zinc-50 mt-2 block tracking-tight group-hover:text-(--accent-color) transition-colors">
                    {infoNumber}
                </span>
            </div>
            
            {/* Detalhe estético opcional: uma linha discreta na base do card ao passar o mouse */}
